...
global network_metadata simulation index_extinct_p index_extinct_a simulation
...
u_ns    = network_metadata.u_ns ;
u_s     = network_metadata.u_s ;
w       = network_metadata.w ;
beta    = network_metadata.beta ;
if simulation ==1
    G   = network_metadata.G ;
else
    G   = 0.*network_metadata.G ; % Without AF
end
g       = network_metadata.g ;
...
gamma = g .* (1 - u_ns' * p - u_s.*p - w .* p + u_ns .* p + u_s.*p);
